...
int64 nChainStartTime = 1403056884;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1){
	       nSubsidy = 150000 * COIN; // first 1 blocks for 0.5%
	       return nSubsidy + nFees;
	  }
	  else if(nHeight >= 2 && nHeight <= 500){
	  	   nSubsidy = 88 * COIN; // 88coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 501 && nHeight <= 1440){
	  	   nSubsidy = 308 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 1441 && nHeight <= 7200){
	  	   nSubsidy = 258 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 7201 && nHeight <= 14400){
	  	   nSubsidy = 208 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 14401 && nHeight <= 21600){
	  	   nSubsidy = 158 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 21601 && nHeight <= 28800){
	  	   nSubsidy = 108 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    int64 nRewardCoinYear;
		...
		if(nHeight < YEARLY_BLOCKCOUNT)
			nRewardCoinYear = 10 * MAX_CLOAK_PROOF_OF_STAKE;
		else if(nHeight < (2 * YEARLY_BLOCKCOUNT))
			nRewardCoinYear = 5 * MAX_CLOAK_PROOF_OF_STAKE;
		else if(nHeight < (3 * YEARLY_BLOCKCOUNT))
			nRewardCoinYear = 2 * MAX_CLOAK_PROOF_OF_STAKE;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Bountycoin is good coin wawawa......";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xbc68bf68ae42168b735acc9136a9e2ec6a9184ca8a744193b67373dc560c4828"));
...
unsigned char pchMessageStart[4] = { 0x73, 0x15, 0x42, 0xf5 };
